...
- `npm run build` — production build (Turbopack off; see `package.json`)
- `npm run start` — run the built app
- `npm run lint` — ESLint
- `npm run update-yields` — `tsx src/scripts/update-yields.ts`
- `npm run check-alerts` — `tsx src/scripts/check-alerts.ts`

Run `npm run lint` and `npm run build` before declaring a code task done.
...
- Branch name: `<author>/<short-slug>` (e.g. `cady/add-eurc-adapter`)
- Title: short, imperative ("Add EURC adapter for Aave v3")
- Body: what changed, why, and a test/QA checklist
- Do **not** open PRs automatically. Wait for the user to ask.
